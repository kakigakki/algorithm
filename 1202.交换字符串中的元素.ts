...
function smallestStringWithSwaps(s: string, pairs: number[][]): string {
  ...
  for (let i = 0; i < s.length; i++) {
    const parentVal: string = uf.find(i) + ""
    if (obj[parentVal]) {
      obj[parentVal].push(i)
    } else {
      obj[parentVal] = [i]
    }
  }
  ...
  for (const key in obj) {
    const sortArr = obj[key].slice()
    sortArr.sort((a, b) => s[a].charCodeAt(0) - s[b].charCodeAt(0))
    for (let i = 0; i < obj[key].length; i++) {
      const index = obj[key][i]
      strArr[index] = s[sortArr[i]]
    }
  }
  return strArr.join("")
